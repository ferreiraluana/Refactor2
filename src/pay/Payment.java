...
public class Payment {
	...
	public static void PassDay(Map<Integer,Employees> employee,int day,int month,int year,int dayofweek,int totalsize) {
		...
		for(int i = 1;i <= totalsize;i++) {
			if(employee.containsKey(i)) {
				if((employee.get(i).getPaymentPaper().getPaymentday() == day) && (employee.get(i).getPaymentPaper().getPaymentmonth() == month) && 
						(employee.get(i).getPaymentPaper().getPaymentyear() == year) && (employee.get(i).getPaymentPaper().getPaymentdayofweek() == dayofweek)) {
					hasemployees++;
					...
					if(employee.get(i).getUnion().getSyndicatestatus().equals("1")) {
						if(employee.get(i) instanceof Employees) {
							System.out.println("Faz parte de Sindicato");
							System.out.println("ID do Sindicato: " + employee.get(i).getUnion().getSyndicateId());
							
							synd.SyndicateCharge(employee, i);
							
							System.out.println("Taxa de Sindicato: " + employee.get(i).getUnion().getSyndicateTax());
							if(employee.get(i).getUnion().getSyndicateServiceTax() != 0) {
								System.out.println("Taxa de Servico do Sindicato: " + employee.get(i).getUnion().getSyndicateServiceTax());
								employee.get(i).getUnion().setSyndicateServiceTax(0);
							}
					...
					System.out.println("Pagamento: " + employee.get(i).getPaymentPaper().getPayment());
					System.out.println("---------------------------------------");
					...
					employee.get(i).getPaymentPaper().setPayment(0);
				}
			}
		}	
		if(hasemployees == 0) {
	...
	public static void SetPaymentSchedule(Map<Integer,Employees> employee,int index) {
		if(employee.get(index).getType().equals("horista")) {
			String currentPaymentSchedule = employee.get(index).getPaymentPaper().getPaymentschedule();
			int DayIWant = 0;
			...
			employee.get(index).getPaymentPaper().setPaymentday(dayp);
			employee.get(index).getPaymentPaper().setPaymentmonth(monthp);
			employee.get(index).getPaymentPaper().setPaymentyear(yearp);
			employee.get(index).getPaymentPaper().setPaymentdayofweek(dayofweekp);
		}
		else if(employee.get(index).getType().equals("assalariado")){
			if(employee.get(index).getPaymentPaper().getPaymentschedule().equals("mensalmente") || employee.get(index).getPaymentPaper().getPaymentschedule().equals("mensal $")) {
				int dayp,monthp,yearp,dayofweekp;
				dayp = employee.get(index).getFirstDay();
				monthp = employee.get(index).getFirstMonth();
				yearp = employee.get(index).getFirstYear();
				...
				String currentSchedule = employee.get(index).getPaymentPaper().getPaymentschedule();
				String now = null;
				int DayOFMonth = 0;
