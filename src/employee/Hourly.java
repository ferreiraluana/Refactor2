...
public class Hourly extends Employees implements Pay{
	...
	public void calculateSalary(Map<Integer,Employees> employee,int employeeindex) {
		...
		if(employee.get(employeeindex).getType().equals("horista")) {
			if(employee.get(employeeindex) instanceof Employees) {
				...
				if(verifypayment == 0) currentemployee.getPaymentPaper().setPayment(0.0);
				else {
					for(int i = 0; i < currentemployee.getPaymentDaily().size();i++) {
						//Try
						try {
					...
					currentemployee.getPaymentPaper().setPayment(finalpaymentsum);
				}
			}
