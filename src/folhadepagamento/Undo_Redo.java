...
public class Undo_Redo {
	static Stack<Undo_Redo> Undo = new Stack<Undo_Redo>();
	static Stack<Undo_Redo> Redo = new Stack<Undo_Redo>();
	String oldoption;
	int oldday,oldmonth,oldyear,olddayofweek,oldtotalsize,oldpositioninarray;
	Map<Integer,Employees> oldmap =  new HashMap<Integer,Employees>();
	
	
	public Undo_Redo(String option1,int totalsize,int positioninarray,Map<Integer,Employees> employee) {
		setOldoption(option1);
		setOldday(Principal.calendar.day);
		setOldmonth(Principal.calendar.month);
		setOldyear(Principal.calendar.year);
		setOlddayofweek(Principal.calendar.dayofweek);
		setOldmap(employee,totalsize);
		setOldpositioninarray(positioninarray);
	}
	
	public static void Save(String option1,int totalsize,int positioninarray,Map<Integer,Employees> employee) {
		Undo_Redo undo_redo = new Undo_Redo(option1,totalsize,positioninarray,employee);
		Undo.push(undo_redo);
	}
	
	public static void SaveRedo(String option1,int totalsize,int positioninarray,Map<Integer,Employees> employee) {
		Undo_Redo undo_redo = new Undo_Redo(option1,totalsize,positioninarray,employee);
		Redo.push(undo_redo);
	}
		...
		for(int currentid = 1;currentid <= totalsize;currentid++) {
			if(employee.containsKey(currentid)) {
				if(employee.get(currentid) instanceof Employees) {
					...
					Employees.addEmployee(this.oldmap, employee.get(currentid).getName(), employee.get(currentid).getAddress(), employee.get(currentid).getType(), employee.get(currentid).getPaymentmethod(), employee.get(currentid).getPaymentPaper().getPaymentschedule(), currentid, employee.get(currentid).getSalary(), employee.get(currentid).getFirstDay(), employee.get(currentid).getFirstMonth(), employee.get(currentid).getFirstYear(), employee.get(currentid).getFirstDayOfWeek(), employee.get(currentid).getUnion().getSyndicatestatus(), employee.get(currentid).getUnion().getSyndicateTax(),commission);
				}
			}
		}
	}
